import { aq as CanvasTarget, F as Fn, P as screenUV, a as vec4, V as vec3, ar as renderOutput, N as NodeMaterial, an as QuadMesh, as as InspectorBase, am as RendererUtils, u as uniform, at as OnMaterialUpdate, a8 as vec2, a7 as step } from "./three.tsl-CucFAqAO-B9znDNZK.js";
import { al as REVISION, b9 as setConsoleFunction, ba as NoToneMapping, bb as TimestampQuery, bc as warnOnce, as as LinearSRGBColorSpace, bd as EventDispatcher } from "./three.core-CntQ0PPt-CIuLIuvN.js";
  ...
  restoreDetachedTabs() {
    ...
    if (needsNewActiveTab) {
      ...
      const availableTabs = tabIds.filter(
        (id) => !this.tabs[id].isDetached && this.tabs[id].isVisible
      );
      if (availableTabs.length > 0) {
        ...
        const orderedTabIds = buttons.map((btn) => {
          ...
          (id) => id !== void 0 && !this.tabs[id].isDetached && this.tabs[id].isVisible
        );
...
class ValueNumber extends Value {
  constructor({ value = 0, step: step2 = 0.1, min = -Infinity, max = Infinity }) {
    super();
    this.input = document.createElement("input");
    this.input.type = "number";
    this.input.value = value;
    this.input.step = step2;
    this.input.min = min;
    this.input.max = max;
  ...
  addDragHandler() {
    ...
    document.addEventListener("mousemove", (e) => {
      if (isDragging) {
        ...
        const step2 = parseFloat(this.input.step) || 1;
        const min = parseFloat(this.input.min);
        const max = parseFloat(this.input.max);
        let stepSize = step2;
        ...
        const precision = (String(step2).split(".")[1] || []).length;
...
class ValueSlider extends Value {
  constructor({ value = 0, min = 0, max = 1, step: step2 = 0.01 }) {
    super();
    this.slider = document.createElement("input");
    this.slider.type = "range";
    this.slider.min = min;
    this.slider.max = max;
    this.slider.step = step2;
    const numberValue = new ValueNumber({ value, min, max, step: step2 });
...
class ParametersGroup {
  ...
  addSlider(object, property, min = 0, max = 1, step2 = 0.01) {
    const value = object[property];
    const editor = new ValueSlider({ value, min, max, step: step2 });
    editor.addEventListener("change", ({ value: value2 }) => {
      object[property] = value2;
    });
    const description = createValueSpan();
...
//# sourceMappingURL=Inspector-DrFBWtkP-DKZg1P_w.js.map
